...
export async function GET() {
  try {
    await connectDB();
    const items = await Slider.find({ isActive: true }).sort({ order: 1 });
    return NextResponse.json({ items });
  } catch (error) {
    console.error("Slider GET error:", error);
    return NextResponse.json({ error: "Failed to load sliders" }, { status: 500 });
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!hasPermission(user, "manage_slider")) {
      return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    }

    await connectDB();
    const body = await req.json();
    const { title, imageUrl, isActive = true, order = 0 } = body;

    if (!title || !imageUrl) {
      return NextResponse.json({ error: "Title and image are required" }, { status: 400 });
    }

    const item = await Slider.create({ title, imageUrl, isActive, order });
...
export async function PUT(req: NextRequest) {
  try {
    ...
    const item = await Slider.findByIdAndUpdate(id, updateData, { new: true });
    if (!item) return NextResponse.json({ error: "Slider not found" }, { status: 404 });
